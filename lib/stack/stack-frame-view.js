...
render () {
    ...
    // Don't display the full absolute path, usually the extra stuff is meaningless.
    const displayedFilePath = projectPaths.reduce((acc, current) => {
        if (acc.startsWith(current)) {
            acc = acc.slice(current.length)
        }
        return acc
    }, frame.filepath)

    const frameTitle = `${frame.id} ${frame.label} ${displayedFilePath} ${'(' + frame.line + ')'}`

    return <li className={highlight} onclick={this.handleClick} title={frameTitle}>
        ...
        <div className='stack-frame-filepath text-smaller inline-block-tight' attributes={{'data-path':frame.filepath}}>{displayedFilePath}</div>
